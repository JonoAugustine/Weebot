...
public class CardsAgainstHumanityCommand extends Command {
    ...
    public static final class CardsAgainstHumanity
        ...
        /**
         * The Cards Against Humanity Player. <br> Has a hand (Array) of {@link WhiteCard
         * White Cards} and won {@link BlackCard Black Cards}.
         */
        final class CAHPlayer extends Player {
            ...
            /**
             * Currently held cards This is visible to the {@link Player} in their
             * privateChannel
             ...
             *
             * @param embed
             *         The new Message to update or send.
             */
            public final void updateHandMessage(MessageEmbed embed) {
                try {
                    this.handMessage.editMessage(embed).queue(
                            message -> this.handMessage = message
                    );
                } catch (Exception e) {
                    super.privateMessage(embed, m -> this.handMessage = m);
                }
            }
        ...
        public static final class CAHDeck {
             ...
             */
            public CAHDeck(String name, Member author, Guild guild)
            ...
            /**
             * Load all cards from the standard deck into this deck.
             */
            ...
            /**
             * Load the cards from other decks;
             *
             * @param decks
             *         the decks to load from.
             *
            ...
            /**
             * @return The deck in a
             * {@link Launcher#makeEmbedBuilder standard Weebot embed}
             */
            private MessageEmbed toEmbed(Guild guild) {
                StringBuilder sb = new StringBuilder()
                        .append("Custom Cards Against Humanity Deck from *")
                        .append(guild.getName()).append("* created by *")
                        .append(Launcher.getJda().getUserById(authorID).getName())
                        .append("*");
                EmbedBuilder eb = Launcher.makeEmbedBuilder(this.name, null, sb);
                eb.setThumbnail(
                        "https://cardsagainsthumanity.com/v8/images/social-3f4a4c57.png");
                sb.setLength(0);
                for (int i = 0; i < this.whiteCards.size(); i++) {
                    sb.append(i + 1).append(".) ").append(this.whiteCards.get(i))
                      .append("\n\n");
                }
                eb.addField("White Cards", sb.toString(), false);

                eb.addBlankField(false);

                sb.setLength(0);
                for (int i = 0; i < this.blackCards.size(); i++) {
                    sb.append(i + 1).append(".) ").append(this.blackCards.get(i))
                      .append("\n\n");
                }
                eb.addField("Black Cards", sb.toString(), false);
             ...
             *         The member.
             *
             * @return true if the member is the author or has a role in {@link
             * CAHDeck#roleLocks}.
             */
        ...
        public static final class WhiteCard extends Card {
            ...
            static final transient File WC_STD = new File("res/CAH/CAH_WHITE_CARDS.card");
        ...
        public static final class BlackCard extends Card {
            ...
            static final class TooManyBlanksException extends InvalidCardException {
                ...
                TooManyBlanksException() {
                    super();
                }
            }

            /** The file of the standard black cards */
            static final transient File BC_STD = new File("res/CAH/CAH_BLACK_CARDS.card");
        ...
        /**
         * Initialize a new CardsAgainstHumanity game with custom decks.
         *
         * @param bot
         ...
         * black card. 3.) Set next Czar 4.) Set game state to {@link GAME_STATE#CHOOSING
         * CHOOSING}
         */
        protected void setupNextRound() {
            ...
            } while (temp.getUser().isBot()
                    || (temp == czar && playerList.size() > 2 /*4 dev vs bot test*/));
            czar = temp;
        ...
        /**
         * Send the player their hand of cards.
         * @param player The player
         */
        protected void sendHand(CAHPlayer player) {
            if(!player.getUser().isBot()) {
                StringBuilder sb = new StringBuilder();
                EmbedBuilder eb = Launcher.getStandardEmbedBuilder();

                sb.append("Cards against ").append(channel.getGuild().getName())
                  .append(" | round ").append(round);
                eb.setAuthor(sb.toString(), null, channel.getGuild().getIconUrl());
                sb.setLength(0);
                ...
                for (int i = 0; i < player.hand.length; i++) {
                    sb.append((i + 1) + ".) " + player.hand[i] + "\n\n");
                }

                eb.addField("Your Deck", sb.toString(), false);

                player.updateHandMessage(eb.build());
            }
        }
         ...
         *         card indices
         *
         * @return 0 if the cards were played <br> -1 if the player has already played
         * their cards<br> -2 if the player is the czar
         */
        ...
        /**
         * Check if all players have played their cards. If so, change {@link GAME_STATE
         * GAME STATE} to {@link GAME_STATE#READING READING}.
         *
        ...
        private final MessageEmbed playedCardsEmbed() {
            ...
            eb.setDescription(
                    "All the players have played their cards" + " -- Czar, please " +
                            "choose your victor.");
        ...
        private MessageEmbed blackCardEmbed() {
            ...
            //3. Arg: icon url as string (can be null)
            eb.setAuthor(
                    "Cards against " + channel.getGuild().getName() + " | round " + round,
            ...
            eb.addField("How to Play:",
                        "Select your cards from the private chat I sent you. \n" +
                                "*cah play <card_number> " +
                                "/card2_number/.../card5_number/*",
                        false
            );
        ...
        private MessageEmbed winnerEmbed(CAHPlayer winner) {
            ...
            //2. Arg: URL as string or could also be null
            eb.setTitle(
                    winner.member.getEffectiveName() + " wins this round!!!:tada: :tada:",
                    null
            );
        ...
        public boolean startGame() {
            if(this.PLAYERS.size() < MIN_PLAYERS && !this.hasDev()) {
                return false;
            }
            //Deal cards
            for (CAHPlayer p : this.PLAYERS.values())
    ...
    private static final String NO_GAME_FOUND = "There is no Cards Against Humanity " +
            "game setup or running! Use ```" + "cah setup [hand_size] [deck_name] " +
            "[deck2_name]...```to setup a new game.";

    private enum ACTION {
        /** Setup a new game */
        SETUP, /** Start the game */
        START, /** Join the game */
        JOIN, /** Player lease the game */
        LEAVE, /** Invite the bot to play */
        ADDBOT, /** End the game */
        END, /** Play cards */
    ...
    protected void execute(Weebot bot, BetterMessageEvent event) {
        ...
        if(action != ACTION.DECKFILE && action != (ACTION.MAKEBLACKCARD) && action !=
                (ACTION.MAKEWHITECARD) && action != (ACTION.MAKEDECK) && action !=
                ACTION.VIEWALLDECKS && action != ACTION.VIEWDECK) {
            for (Game g : bot.getRunningGames()) {
                if(g instanceof CardsAgainstHumanity) {
                    if(((CardsAgainstHumanity) g).channel.getIdLong() == channel
                            .getIdLong()) {
        ...
        switch (action) {
            case SETUP: //cah setup [hand_size] [deck_name] [deck2_name]...
                if(game == null) {
                    ...
                    List<CAHDeck> reqDecks = new ArrayList<>();
                    ...
                    cahDeck = new CAHDeck().loadFrom(reqDecks);
                    //If there aren't enough custom cards, load the standard deck
                    if(cahDeck.whiteCards.size() < 100 || cahDeck.blackCards
                            .size() < 70) {
                        cahDeck.loadStandard();
                    }
                    game = new CardsAgainstHumanity(bot, channel, event.getMember(), hs,
                                                    cahDeck);
                    bot.addRunningGame(game);
                    sb.append("A new game of *Cards Against ")
                      .append(event.getGuild().getName()).append("* has been setup!")
                      .append("```cah join```to join the game.\n")
                      .append("```cah start```to start the game (Minimum of 3 players");
                    ...
                    sb.append("There is already a game of Cards Against Humanity ")
                      .append("being played in this text channel. Please end that")
                      .append("game before starting a new one, or setup a ")
                      .append("new game in a different text channel.");
                    event.reply(sb.toString());
                }
                return;
            case START:
                if(game != null) {
                    if(game.isRunning()) {
                        event.reply(
                                "There is already a game of Cards Against Humanity" + "" +
                                        " running.",
                                m -> {
                                    try {
                                        Thread.sleep(5 * 1000);
                    ...
                    if(game.startGame()) {
                        ...
                        event.reply(
                                "You need at least **3** players to start a game of" +
            ...
            case ADDBOT:
                if(game != null) {
                    Member botMember = event.getGuild()
                ...
                if(game != null) {
                    ...
                    if(args.length >= 2 + game.blackCard.blanks) {
                        ...
                        try {
                            ...
                            e.printStackTrace();
                            event.reply(
                                    "Sorry, I had some trouble reading your request" +
                                            ".");
                            return;
                        ...
                        switch (game
                                .playCards(game.getPlayer(event.getAuthor()), cards)) {
                            case 0:
                        ...
                        event.reply(
                                "Not enough cards. Choose " + game.blackCard.blanks + "" +
                                        " cards.",
                                m -> {
                                    try {
                                        Thread.sleep(5 * 1000);
            ...
            case PICK: //cah pick <card_set_num>
                if(game != null) {
                    if(game.STATE != GAME_STATE.READING) {
                        ...
                        //clear clutter
                        event.reply(
                                "*Players are still choosing thier cards, please " +
                    ...
                    if(!game.isCzar(event.getMember())) {
                        event.reply("Only the Card Czar, **" + game.czar.member
                                .getEffectiveName() + "** can pick a winner.", m -> {
                            try {
                                Thread.sleep(2 * 1000);
                            } catch (InterruptedException e) {
                            }
                    ...
                    try {
                        ...
                        event.reply(
                                "Please choose one of the listed players by " + "their " +
                                        "number");
                        return;
                    }

                    if(winner == game.czar) {
                        event.reply("*Please select one of the players listed*", m -> {
                            try {
                                Thread.sleep(5 * 1000);
                            } catch (InterruptedException e) {
                            }
            ...
            case VIEWDECK: //cah viewdeck <deck_name>
                ...
                event.privateReply(cahDeck.toEmbed(event.getGuild()));
                event.deleteMessage();
                return;
            case MAKEDECK: //cah mkdk <deck_name>
                ...
                try {
                    deck = new CAHDeck(
                            ...
                            event.getMember(), event.getGuild()
                    );
            ...
            case MAKEBLACKCARD: //cah mkbc <deck_name> <blanks> <card text>
                ...
                if(cahDeck.addBlackCard(blackCard)) {
                    sb.append("The Black Card was added to deck *").append(cahDeck.name)
                      .append("*");
                    event.reply(sb.toString());
                } else {
            ...
            case REMOVEWHITECARD: //cah rmwc <deckname> <card_number>
                int cardIndex;
                try {
                    cardIndex = Integer.parseInt(args[3]) - 1;
                    cahDeck = bot.getCustomCahDeck(args[2]);

                } catch (IndexOutOfBoundsException | NumberFormatException e) {
                    sb.append("Please provide a deck name and")
                      .append(" whitecard number like this:")
                      .append("```cah rmwc <deck_name> <card_number>");
                    event.reply(sb.toString());
                    return;
                }
                if (cahDeck == null) {
                    sb.append("I could not find a deck named '*").append(args[2])
                      .append("*'. Make sure the deck name has any spaces replaced with")
                      .append("underscores ( _ ).");
                    event.reply(sb.toString());
                    return;
                }

                try {
                    cahDeck.whiteCards.remove(cardIndex);
                } catch (IndexOutOfBoundsException e) {
                    sb.append(cardIndex).append(" is not an available card.");
                    event.reply(sb.toString());
                    return;
                }

                event.reply("*The whitecard was removed.*");
                return;
            case REMOVEBLACKCARD:
                try {
                    cardIndex = Integer.parseInt(args[3]) - 1;
                    cahDeck = bot.getCustomCahDeck(args[2]);
                ...
                try {
                    cahDeck.blackCards.remove(cardIndex);
                } catch (IndexOutOfBoundsException e) {
                    sb.append(cardIndex).append(" is not an available card.");
                    event.reply(sb.toString());
                    return;
                ...
                event.reply("*The blackcard was removed.*");
                return;
            case DECKFILE: //cah deckfile [deck_name]
                try {
                    ...
                    if(cahDeck == null) {
                        if(args[2].equalsIgnoreCase("default") || args[2]
    ...
    private ACTION parseAction(String arg) {
        switch (arg.toLowerCase()) {
            ...
            case "rmbc":
                return ACTION.REMOVEBLACKCARD;
            case "rmwc":
                return ACTION.REMOVEWHITECARD;
            case "rmdk":
            case "bin":
    ...
    private static final CAHDeck loadStandardDeck() {
        ...
        Scanner scanner;
        try {
            Files.readAllLines(WhiteCard.WC_STD.toPath()).iterator()
                 .forEachRemaining(line -> deck.whiteCards.add(new WhiteCard(line)));
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public MessageEmbed getEmbedHelp() {
        ...
        eb.setThumbnail("https://cardsagainsthumanity.com/v8/images/social-3f4a4c57.png");
